...
class ChatConsumerTestCase(TestCase):
    # Wraps the setUp method to allow database operations
    @database_sync_to_async
    def async_setup(self):
        async def test_chat_message_broadcast(self):
            # Calls async_setup to create test data in an async-safe way
            await self.async_setup()
            # Creates a chat room and connect two users
            chat_room = ChatRoomFactory()
            user1 = UserFactory()
            user2 = UserFactory()
            chat_room.members.add(user1, user2)

            communicator1 = WebsocketCommunicator(
                ChatConsumer.as_asgi(), f'/chat/{chat_room.chat_name}/')
